...
const controlDetailed = (req, res) => {
  ...
  getPermissions("can_view", id, resource)
    .then((canView) => {
      if (!canView) {
      ...
      pool.getConnection(function (err, connection) {
        const sql = `Select DISTINCT student_id, CONCAT(students.student_name, ' ', students.student_surname) as student_name_surname, group_name from ${resource} inner join students on students.ID_student = ${resource}.student_id inner join groupss on groupss.ID_group = students.group_id;`;
